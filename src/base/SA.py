...
def SA(fitness_function, problem_dim: int, bounds: list, epochs: int, trials: int, initial_probability: float,
        final_probability: float, verbose: bool = False):

    x = np.array([bounds[0]] * problem_dim) +\
        np.array([abs(bounds[1] - bounds[0])] * problem_dim) *\
        np.random.rand(problem_dim)  # initial population

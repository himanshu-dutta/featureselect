...
class GA:
    def __init__(self, fitness_function, population_size: int, chromosome_size: int, bounds: list,
                 survival_rate: float, mutation_ratio: float, generation_count: int):
        ...
        self.population = np.array([bounds[0]] * chromosome_size) +\
            np.array([abs(bounds[1] - bounds[0])] * chromosome_size) * np.random.rand(
            self.population_size, self.chromosome_size)

    def calc_fitness(self):
        self.fitness = np.array([self.fitness_function(chromosome)
    ...
    def crossover(self):
        ...
        offspring_size = (
            int(np.ceil((1-self.survival_rate)*self.population_size)), self.chromosome_size)
        ...
        for i in range(offspring_size[0]):
            ...
            crossover_point = int(
                np.floor(np.random.rand() * self.chromosome_size))
    ...
    def mutation(self):
        mutation_size = int(
            np.floor(self.mutation_ratio * self.chromosome_size))

        for child in range(self.children.shape[0]):
            genes = np.random.choice(self.chromosome_size, mutation_size)
            for gen_pos in genes:
                self.children[child, gen_pos] += np.random.choice(
                    [-1.0, 1.0]) * self.fitness[child % self.parents.shape[0]]
    ...
    def run(self, verbose: bool = False):
        if verbose:
            ...
            print(f"Initial Accuracy: %.3f." %
                  (1-self.fitness_function(all_featues)))

        for gen in range(self.generation_count):
            if verbose:
                print("----------------------------------")
                print("*  Epoch: {epoch:{width}}".format(epoch=gen +
                                                         1, width=len(str(self.generation_count))), end=" | ")

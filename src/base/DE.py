...
def DE(fitness_function, bounds: list, dimension: int, target_size: int,
       epochs: int, scaling_factor: float, crossover_prob: float, threshold_epochs: int = None, verbose: bool = False):

    target_vector = np.array([bounds[0]] * dimension) +\
        np.array([abs(bounds[1] - bounds[0])] * dimension) * np.random.rand(
        target_size, dimension)  # target vector
    ...
    for epoch in range(epochs):
        ...
        for pop in range(target_size):
            ...
            tv_row = [doner_vector[i] if np.random.rand() <= crossover_prob or delta ==
                      i else target_vector[pop, i] for i in range(dimension)]
            trial_vector.append(tv_row)

        trial_vector = np.clip(np.array(trial_vector), bounds[0], bounds[1])
        trial_fitness = np.array([fitness_function(pop)
                                  for pop in trial_vector])
        ...
        if threshold_epochs and len(iter_fitness) >= threshold_epochs and np.all(iter_fitness[-1] == iter_fitness[-threshold_epochs:]):
            break
